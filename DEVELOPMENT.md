...
Publish to the VS Code Marketplace from a terminal:
...
2. Run the publish wrapper with the right version bump:
   ...
   Use `publish:minor` or `publish:major` instead when the change warrants it. Use `npm run publish:vsix` to publish without bumping the version.

   The wrapper calls the existing package flow, finds the newest generated `.vsix`, runs `npx vsce publish --packagePath <file>`, then verifies the Marketplace item with:
