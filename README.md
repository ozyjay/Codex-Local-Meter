...
Install dependencies with `npm install`, then use these scripts from the repository root:

| Command | What it does |
| --- | --- |
| `npm run compile` | Type-checks TypeScript and writes the normal extension build to `out/`. |
| `npm run watch` | Rebuilds `out/` as source files change. |
| `npm run lint` | Checks TypeScript style with ESLint. |
| `npm run unit-test` | Runs the compiled logic-focused tests from `out/` without starting VS Code. Run `npm run compile` first. |
| `npm test` | Compiles a temporary `.test-out/` extension and runs the full VS Code extension-host suite. This avoids conflicts when VS Code has files in `out/` open. |
| `npm run package:vsix` | Builds a local `.vsix` package. |
| `npm run package:patch`, `package:minor`, or `package:major` | Builds a local `.vsix` after calculating and applying that version bump. |
| `npm run publish:vsix` | Packages the current version, publishes it to the VS Code Marketplace, and verifies the published item. |
| `npm run publish:patch`, `publish:minor`, or `publish:major` | Applies that version bump, packages, publishes, and verifies the Marketplace item. |

The temporary `.test-out/` folder is generated by the test script and is ignored by Git. The full test runner may report harmless VS Code/Electron environment warnings; the test result is successful when it ends with `passing` and exit code `0`.

Publishing requires VS Code Marketplace credentials. Run `npx vsce login CrunchyCodes` beforehand, or set `VSCE_PAT` in the shell. The publish scripts make an external Marketplace release, so use them only when you intend to publish.
