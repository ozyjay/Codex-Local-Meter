...
Files in `images/` are either hand-authored sources or generated outputs:

- `images/icon-mono.svg` — hand-authored source for the status bar glyph. Must use plain filled paths only (no stroke, no colour) so `svgtofont` can embed it as a font glyph. Do not add stroke or colour attributes.
- `images/icon.svg` — hand-authored source for the marketplace/extension thumbnail. Full-colour SVG.
- `images/codex-local-meter.woff` — generated by `scripts/RebuildVsix.ps1` from `icon-mono.svg` via `svgtofont`. Do not hand-edit. The tool is non-deterministic: the binary changes on every run even with identical input, so only commit this file when `icon-mono.svg` has actually changed.
- `images/icon.png` — generated from `icon.svg` via `npx @resvg/resvg-js-cli images/icon.svg images/icon.png`. Not regenerated by `RebuildVsix.ps1`; regenerate manually after editing `icon.svg`.
